...
function get51ResumeHtml(html,itemId,memberId){
	...
	try{
		...
		if(html.indexOf("/resources/500d/cvresume/js/9a7d8bd1-feb2-464a-bcee-447a1d574091-thumbnail.jpg")!=-1){
			$("#importRModal").modal("hide");
			$("#importResetModal").modal("show");
			$("#importResetModal").find(".tips_show").text("当前导入的简历是智联简历，请选择前程无忧简历再导入");
			return;
		}else if(html.indexOf("/resources/500d/cvresume/js/1246.html")!=-1){
			$("#importRModal").modal("hide");
			$("#importResetModal").modal("show");
		...
		var custom = []; 						//自定义模块数组
		//----51job必填基本信息部分----
		var Basic=resume_page.find("/resources/500d/cvresume/js/9fb69866-d1fa-45f7-9e19-5415ea0c6879-thumbnail.jpg");
		//姓名
		...
		var boxs=resume_page.find("/resources/500d/cvresume/js/1260.html");
		boxs.each(function(index,ele){
			...
			if(titleName.indexOf("个人信息")!=-1){
				...
				$infos.each(function(index,ele_item){
					//messages[uuid()]=clearAllHtmlText($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text()+$(ele_item).find("/resources/500d/cvresume/js/1247.html").text());
					//qq
					if($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text().indexOf("QQ号")!=-1){
						resume_base_info["qq"]=$(ele_item).find("/resources/500d/cvresume/js/1247.html").text().trim();
					//微信
					}else if($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text().indexOf("微信")!=-1){
						resume_base_info["weixin"]=$(ele_item).find("/resources/500d/cvresume/js/1247.html").text().trim();
					//身高
					}else if($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text().indexOf("身高")!=-1){
						var height = {
								"key":uuid(),
								"name":"height",
								"desc":$(ele_item).find("/resources/500d/cvresume/js/1247.html").text().trim()
						};
						customMsg.push(height);
					//婚姻状况
					}else if($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text().indexOf("婚姻")!=-1){
						var marriageStatus = $(ele_item).find("/resources/500d/cvresume/js/1247.html").text().trim();
						resume_base_info["marriageStatus"]=getMarriageStatus(marriageStatus);
					///政治角色
					}else if($(ele_item).find("/resources/500d/cvresume/js/e3b0300d-758a-47d7-9fd2-469aca786507-thumbnail.jpg").text().indexOf("政治")!=-1){
						var politicalStatus = $(ele_item).find("/resources/500d/cvresume/js/1247.html").text().trim();
...
function getLagouResumeJson(html,itemId,memberId){
	...
	try{
		...
		work_eles.each(function(j, ele_item) {
			...
			item["beginTime"] = time[0].trim();			//获取时间"/resources/500d/cvresume/js/df44f922-344a-4b04-83dc-d6cc8bb87362-thumbnail.jpg", 并且去掉空格
			item["endTime"] = time[1].trim();			//获取时间"/resources/500d/cvresume/js/1248.html", 并且去掉空格
...
function getZhilianResumeJson(html,itemId,memberId){
	...
	try{
		var resume_page=$(html);
		if(html.indexOf("/resources/500d/cvresume/js/1246.html")!=-1){
			$("#importRModal").modal("hide");
			$("#importResetModal").modal("show");
			$("#importResetModal").find(".tips_show").text("当前导入的简历是拉勾简历，请选择智联简历再导入");
			return;
		}else if(html.indexOf("/resources/500d/cvresume/js/9a7d8bd1-feb2-464a-bcee-447a1d574091-thumbnail.jpg")==-1){
			$("#importRModal").modal("hide");
			$("#importResetModal").modal("show");
...
function init_resume(resume,modul_show,iconFontMap){
	...
	resume["resume_qrcode"]={					//二维码
	        "qrCodeImg": "../../images/ad_Attention_weixin_ewm.png"/*tpa=/resources/500d/images/ad_Attention_weixin_ewm.png*/,
	        "qrCodeTips": ""
	    };
