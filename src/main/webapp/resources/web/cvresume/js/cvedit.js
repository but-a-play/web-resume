...
cvmutual.main = {
	...
	event_:function(){// 事件
        $(document).on("click","#importResumeBtn",function(){
		...
		$(document).on("click",".wbdCv-baseStyle .baseItem dt div",function(){
			...
			// var $this =
			// $(this).parents(".baseItem-toolbar").siblings("dl").children("dt").find("div");
			// var $id =
			// $(this).parents(".baseItem-toolbar").parents(".baseItem").attr("id");
			// $(".baseItem-toolbar .set
			// .set-con").css({"height":"0px","overflow":"hidden","transition":"all
			// 0.3s"});
			$this.attr("contentEditable",true).focus();
			// $("div[id ='"+$id+"']").find("dl dt div").focusout(function(){
            $this.focusout(function(){
				// var $title = $("div[id ='"+$id+"']").find("dl dt
				// div").text();
				var $title = $this.text();
	...
	resume_select_template:function(){
        function reload_template_list(){
            ...
            $.get(wbdcnf.base+"/cvresume/select_template.jhtml",{"type":current_resume_type,"resumeBankType":current_resume_bank_type,"pageNumber":select_template_page},function(result){
                if(result.item!=null&&""!=result.item.trim()){
                	$(result.item).appendTo($(".change_template .resume_template"));
		...
		$(document).on('click',"#change_temlate_btn",function(){
			// 1判断是否存在模板节点
		...
		// 模板列表加载更多
        $(".left_editor_modal.change_template .modal-content").on("scroll",function(){
            var contain_height = $(".change_template .resume_template").height()+22+75,
                scroll_top = $(this).scrollTop(),
            outer_height = $(this).height();
            if(contain_height - (scroll_top + outer_height) <= 50 && !loading_template){
...
//			common.main.resumeOperationLogUpload(cvresume.info.resumeid,"changemoban","","");// 日志上报
			...
			location.href=wbdcnf.base+"/cvresume/edit.jhtml?itemid="+id+"&resumeId="+cvresume.info.resumeid+"&language="+cvresume.info.language;
		});
	},
	resume_modal:function(){// modal模态框调用
		...
		$("#icon_search_value").keyup(function(){
			 ...
			 if(!cvresume.main.is_empty(name)){
				$.get(wbdcnf.base + "/icon/findIcon.jhtml",{"name":name},function(result){
					console.log(result);
		...
		$(".l-cvbar a").click(function(e){
			...
			if($my_resume_list.find("li").length<=0){
				...
				$.ajax({
		             type: "POST",
		             url: wbdcnf.base + "/cvresume/select_resume.jhtml",
		             data: {"memberid":cvresume.info.memberid},
		...
		if($(".wbdCv-container").hasClass("mobile")){}else{
			...
			if(pageSize != nowPageSize) {
				...
				for(var index = 1; index < pageSize; index++) {
					...
					pageBreakObj.css({"top" : (index * resumePageHeight) + "px"});
...
//				 })
	...
	set_workItem_img:function(){// 设置作品展示上传图片cvmutual.main.set_workItem_img($(".portfolio-preview
		...
		function addLinkList(){
 		...
 		function uploadImg(){
			...
			$.post(wbdcnf.base + '/file/upload/cropper_image.jhtml',{"token" : getCookie("token"),"cropper_image":$image.cropper("getCroppedCanvas").toDataURL("image/jpeg")}, function(result){
				if(result == "error") {
					alert("修改失败！");
					...
					alert("上传图片请先登录！");
			 		...
			 		upload_image_url=wbdcnf.base +result;
			 		addWorkList();
			 	}
	...
	set_headItem_img:function(cropper_img,uploadimg){// 设置头像上传
		...
		$("#headimg-modal .button.submit").click(function(){
			...
			var image_cropper_data=image_cropper.toDataURL("image/jpeg");			
			 // 把裁剪好的图片上传
			$.post(wbdcnf.base + '/file/upload/cropper_image.jhtml',{"token" : getCookie("token"),"cropper_image":image_cropper_data.toString()}, function(data){
				result = data.message;
				if(result == "error") {
					alert("修改失败！");
				} else if(result == "notlogin") {
					alert("上传头像请先登录！");
			 	} else if(result == "ntosuport") {d
			 		alert("文件格式不支持！");
			 	} else if(result == "not_data") {
			 		layer.msg("图片太大，请压缩处理后继续！");
			 	} else {
    ...
    set_skillItem:function(){
        ...
        $(document).on("click",".skillItem .edit,.skillItem .baseItem-null,.skillItem .skill-title",function(){
            ...
            if($("#skillistBody a").length==0){
            	$.get(wbdcnf.base + "/cvresume/skill.jhtml",function(result){
				if(result!=null&&result!=""){
	...
	set_hobbyItem:function(){
        function addHobbys(icon,name){
    	   // var $Hicon = eval("hobbyIcon." + text) == undefined ? "&#xe70f;"
			// : eval("hobbyIcon." + text);
    	    // console.log($Hicon);
...
//           $.ajax({
//	             type: "GET",
//	             url: wbdcnf.base +"/cvresume/hobby/updateNum.jhtml",
//	             data:{"id":id},
//	             success: function(result){
//	            	if(result.type == "success"){
//	            	
...
//			 })
        ...
        $("#hobbys-modal .skillInput").on('keypress',function(event){
	        ...
	        $(document).on("click",".hobbyItem .edit,.hobbyItem .baseItem-null,.hobbyItem .hobby-title",function(){
	            ...
	            if($("#hobby a").length==0){
	            	$.get(wbdcnf.base + "/cvresume/hobby.jhtml",function(result){
	            		if(result!=null&&result!=""){
	            			$("#hobby").append(result);
	            			if($("#hobby a").length!=0 ){
    ...
    get_infoToModal:function(){
        ...
        if($(".info-defind").length > 0){
            ...
            // var inner = "<div data-panel='defind' class='add'><input
			// type='text' placeholder='字段名称' class='defindName'
			// maxlength='5'><input type='text' placeholder='字段内容不超过20个字'
			// maxlength='20' class='defindContent'><a href='javascript:;'
			// class='closeDefind'></a></div><a href='javascript:;'
			// class='openDefind' id='openDefind'>自定义字段</a>";
            // $(".defindItem").html(inner);
        }       // defind
	...
	resume_diagnose:function(){
 	    ...
 	    function diagnose(){
 	        /**
			 * 针对分数 type：default 针对模块 type：modal_title { type:"modal_title",
			 * modal_title:"【头像】", content:"还没有上传照片哦！}
			 */
            ...
            if(_grade > 80){
                ...
                }else if(_resume.resume_base_info.name === "你的名字"){
                    _diagnose_arr.push({
                        type:"modal_title",
                        modal_title:"【基本信息】",
